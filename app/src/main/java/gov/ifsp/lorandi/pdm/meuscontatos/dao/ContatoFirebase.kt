...
class ContatoFirebase: ContatoController() {
    ...
    fun createContato(contato: Contato) = createOrUpdateContato(contato)

    fun readContato(nome: String): Contato = contatosList[contatosList.indexOfFirst { it.nome.equals(nome) }]

    fun readContatos(): MutableList<Contato> = contatosList

    fun updateContato(contato: Contato) = createOrUpdateContato(contato)
